...
@Service
public class LandLineMonthlyReceiptLogic implements ServicesProvider{
    private static List<LandLine> landLineList;

    public LandLineMonthlyReceiptLogic(){
    ...
    public String AddServices(LandLine landLine) {
        for (LandLine LandLineDb:landLineList) {
            if(LandLineDb.getId()==landLine.getId())
                return "Its Land Line Monthly Receipt Services is payed Successfully before";
        }
        landLineList.add(landLine);
        return "Land Line Monthly Receipt Services payed Successfully...";
    }
